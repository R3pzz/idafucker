...
namespace fuse
{
[[nodiscard]] std::wstring ToUnicode::impl(const char* data, const std::size_t size) {
#if FUSE_PLATFORM_WIN
  std::wstring converted{};

  const auto convertedSize = ::MultiByteToWideChar(CP_UTF8, NULL, data,
...
[[nodiscard]] std::string ToUtf8::impl(const wchar_t* data, const std::size_t size) {
#if FUSE_PLATFORM_WIN
  ...
  converted.resize(static_cast<std::size_t>(convertedSize));
  if (::WideCharToMultiByte(CP_UTF8, NULL, data, size, converted.data(), converted.size(),
                            NULL, NULL) < 0)
    throw std::runtime_error{"bad multibyte cast"};
